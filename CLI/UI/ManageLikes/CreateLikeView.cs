...
public class CreateLikeView (ILikeRepository likeRepository, IUserRepository userRepository, IPostRepository postRepository, ICommentRepository commentRepository) : IView
{
    public async Task Run()
    {
        try
        {
            ...
            else if (likeableTypeStr == typeof(Comment).ToString())
            {
                await commentRepository.GetByIdAsync(likeableId);
            }
